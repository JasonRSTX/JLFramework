...
@interface NSString (JLStringSize)
...
@interface NSString (JLStringMatchesRegular)
/**
 判定字符串是否通过正则表达式约束
...
@interface NSString (JLStringFillToLenght)
/**
 向字符串分前后添加指定长度的字符
...
@interface NSString (JLStringMetaHTML)
/**
 将字符串添加HTML标签，支持屏幕适配
...
@interface NSString (JLStringEncryption)
